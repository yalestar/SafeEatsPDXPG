  ...
  class << self
    ...
    def run_parser
      ...
      inspection_links.each do |i|
        ...
        inspection_links = restaurant_page.links.select { |l| l.href =~ /rim\.jsp\?q_ID=\d+/ }
        inspection_links.each do |i|
          # puts "\t\t#{i.href}"
          inspection = fetch_inspections(i.href)
        end
    ...
    def fetch_inspections(url)
      ...
      agent = Mechanize.new
      inspection_page = agent.get("http://web3.clackamas.us/healthapp/#{url}")

      # puts "visiting #{i.href}"
      bottom_table = inspection_page.search("//table[@border='0']/tr[2]/td/table")
